...
export default function HomeScreen() {
  ...
  const checkAchievementsAndCelebrate = async () => {
    ...
    try {
      ...
      // Check for newly unlocked achievements in celebration queue
      ...
      if (queueData) {
        ...
        if (queue.length > 0) {
          // Process latest achievement for celebration
          const latest = queue[queue.length - 1];
          if (latest.achievement) {
            // Celebration will be triggered by AchievementCelebrationProvider
            // Just clear the queue after a delay
            setTimeout(async () => {
  ...
  const togglePrayer = async (index: number) => {
    ...
    // Check for achievements after completing a prayer
    if (updatedPrayers[index].completed && user?.id) {
      try {
        await checkAndUnlockAchievements(user.id);
        // Check for celebrations after a short delay
        setTimeout(() => {
          checkForUncelebratedAchievements(user.id);
        }, 500);
      } catch (error) {
        console.log('Error checking achievements after prayer:', error);
      }
    }
